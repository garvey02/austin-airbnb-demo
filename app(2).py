...
@st.cache_data
def load_data():
    """Load and clean the listings CSV. Searches multiple paths/filenames."""
    import glob
    import os

    # All possible filenames and paths to check
    possible_names = [
        'listings_1_.csv', 'listings(1).csv', 'listings.csv',
        'listing.csv', 'Listings.csv', 'listings_1.csv',
        'listings (1).csv', 'data/listings.csv', 'data/listings_1_.csv',
    ]

    # Also search for any CSV with 'listing' in the name
    glob_patterns = ['*listing*.csv', '**/*listing*.csv', '*.csv']
    ...
    if df is None:
        for pattern in glob_patterns:
            ...
            for match in matches:
                try:
                    test_df = pd.read_csv(match)
                    if 'price' in test_df.columns and 'neighbourhood' in test_df.columns:
                        df = test_df
                        st.sidebar.success(f"Loaded: {match}")
                        break
                except Exception:
                    continue
            if df is not None:
    ...
    if df is None:
        st.error("❌ Could not find the listings CSV file.")
        st.markdown("""
        **To fix this**, make sure your CSV file is in your GitHub repo at the same level as `app.py`.
        ...
        ├── app.py
        ├── requirements.txt
        └── listings_1_.csv   ← rename your CSV to this
        ```
